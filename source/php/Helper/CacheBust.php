...
class CacheBust
{
    ...
    {
        $jsonPath = MODULARITYRESOURCEBOOKING_PATH . apply_filters('ModularityResourceBooking/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . MODULARITYRESOURCEBOOKING_PATH . ' and run gulp. See '. MODULARITYRESOURCEBOOKING_PATH . 'README.md for more info.</div>';
        }
    }
}
