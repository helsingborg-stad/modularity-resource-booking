@if($data['apiKey'])
        ...
        function initGoogleMap() {
            ...
            if (isUndefined(jsonData.getPackageData)) {
                for (let item in jsonData.getPackageData) {
                    ...
                    info[item] += (isUndefined(jsonData.getPackageData[item].title) && jsonData.getPackageData[item].title !== '') ?
                        '<h3>' + jsonData.getPackageData[item].title + '</h3>' : '';
                    info[item] += (isUndefined(jsonData.getPackageData[item].content) && jsonData.getPackageData[item].content !== '') ?
                    ...
                    for (let prodItem in jsonData.getPackageData[item].productSpec.value) {
                        ...
                        info[item] += (isUndefined(jsonData.getPackageData[item].productSpec.value[prodItem].media_name) &&
                            ...
                            : '';
                        info[item] += (isUndefined(jsonData.getPackageData[item].productSpec.value[prodItem].media_type) &&
                            jsonData.getPackageData[item].productSpec.value[prodItem].media_type != '') ? '<p><span>'
    ...
    <script async defer
            src="https://maps.googleapis.com/maps/api/js?key=<?php echo $data['apiKey']; ?>&callback=initGoogleMap"></script>
@endif
